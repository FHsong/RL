...
if __name__ == '__main__':
    ...
    env = Maze()
    Q_learning = QLearningTable(env, learning_rate, discount_factor, epsilon)

    for episode in range(200):
        env.current_state = env.reset()
        env.render()
        while True:
            action = Q_learning.choose_action_e_greedy(env.current_state)

            next_state, r, done = env.step(action)

            Q_learning.learn(env.current_state, action, r, next_state)
